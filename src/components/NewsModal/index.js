...
import { postJSON, postFormData, WEB_DAMRI, BUCKET } from '../../api/utils'
...
export default function NewsModal(props = defaultProps){
    ...
    async function _getDetail(){
        ...
        try {
            const res = await postJSON(`/marketingSupport/articleLink/list`, params, appContext.authData.token)
    ...
    async function _submitData(image, linkPdf = null){
        ...
        try{
            ...
            const result = await postJSON('/marketingSupport/articleLink/'+typeUrl, query, appContext.authData.token)
